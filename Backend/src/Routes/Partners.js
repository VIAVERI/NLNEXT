...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query('SELECT * FROM partner WHERE partner_id = $1', [id]);
...
router.get('/:id/with-services', async (req, res) => {
    try {
        ...
        console.error(err);
        res.status(500).json({ error: 'An error occurred while fetching the partner with services' });
    }
});
...
router.post('/:id/services', async (req, res) => {
    try {
        ...
        console.error(err);
        res.status(500).json({ error: 'An error occurred while adding the service to the partner' });
    }
});
